...
def train_agents(visualize_training=True, render_speed=2.0):
    ...
    for episode in range(episodes):
        ...
        while not done:
            ...
            state_tensor = torch.FloatTensor(thief_agent._preprocess_state(state))
            with torch.no_grad():
                value = thief_agent.value(state_tensor).item()
            thief_agent.remember(thief_reward, value, done)
            
            # Train agents
            police_loss = police_agent.replay(batch_size)
            ...
            if episode % update_target_every == 0:
                police_agent.update_target_model()
    ...
    # Save models
    torch.save(police_agent.model.state_dict(), 'models/police_dqn.pth')
    torch.save({
